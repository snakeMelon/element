    ...
    createMonthSlide(h) {
      return this.displayDates.map((displayDate, index) => {
        const displayMonthDay = this.type === 'date'
          ? this.dateTimeFormat.formatDisplay(displayDate)
          : this.dateTimeFormat.getMonthList()[displayDate.getMonth()];
        return h('transition', {
          props: {
            name: `mu-date-display-${this.slideType}`
          },
          ...
          h('div', {
            ...
            h('div', {staticClass: 'mu-date-display-monthday-title'},
              displayMonthDay)
          ])
        ]);
      });
